...
public class XmlInstanceInspector {
    ...
    private void mapParentNode(Node node, XmlComplexType parent) {
        ...
        XmlField[] existing = parent.getXmlFields().getXmlField().stream()
                .filter(f -> f.getName().equals(node.getNodeName()) && f.getFieldType() == FieldType.COMPLEX)
                .toArray(XmlField[]::new);
