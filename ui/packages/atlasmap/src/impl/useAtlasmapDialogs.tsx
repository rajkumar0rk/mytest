...
*/
import { IConstant, IDocumentName, INamespace, IProperty,IDocument } from '../UI';
...
export function useAtlasmapDialogs({
  ...
  //#region source dialogs
  const [createSourceDialog, openCreateSourceDialog] =
  useDocumentDialog("Source",flatSources);
const onCreateSource = useCallback(
  (docNameInfo: IDocument) => {
    openCreateSourceDialog(async({ id,type,value,valueType,selectValue,valueKey,isSource}) => {

      changeDocument(id, type,value,valueType,selectValue,valueKey,isSource);
    }, docNameInfo);
  },
  [ changeDocument,openCreateSourceDialog],
);
  //#endregion

   //#region target dialogs
   const [createTargetDialog, openCreateTargetDialog] =
   useDocumentDialog("Target",flatTargets);
 const onCreateTarget = useCallback(
   (docNameInfo: IDocument) => {
     openCreateTargetDialog(async({ id,type,value,valueType,selectValue,valueKey,isSource}) => {
 
       changeDocument(id, type,value,valueType,selectValue,valueKey,isSource);
     }, docNameInfo);
   },
   [changeDocument,openCreateTargetDialog],
 );
   //#endregion
